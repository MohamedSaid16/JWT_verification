...
class RoleService {
     ...
     */
    async getAllRoles() {
        return Object.values(client_1.Role).map(buildRoleSummary).sort((a, b) => b.level - a.level);
    }
    /**
    ...
    async removeRoleFromUser(userId, roleId) {
        const normalizedUserId = asString(userId);
        const role = toRole(asString(roleId));
        const user = await database_1.default.user.findUnique({ where: { id: normalizedUserId }, select: { role: true } });
        if (!user) {
        ...
        await database_1.default.user.update({ where: { id: normalizedUserId }, data: { role: client_1.Role.STUDENT } });
        return { changed: true };
    }
    /**
     * Get user roles
    ...
    async getUserRoles(userId) {
        const user = await database_1.default.user.findUnique({ where: { id: asString(userId) }, select: { role: true } });
        if (!user)
            return [];
     ...
     */
    async syncRolePermissions(roleId, permissionIds, grantedBy) {
        return {
            roleId: asString(roleId),
            permissionIds,
            grantedBy,
     ...
     */
    async getUsersWithPermissions(userIds) {
